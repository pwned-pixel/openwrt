...
platform_do_upgrade() {
	...
	zte,mf269)
		...
		;;
	zyxel,nbg7815)
		local mtd_part_2="mtd""$(find_mtd_index 0:bootconfig)"
		local mtd_part_3="mtd""$(find_mtd_index 0:bootconfig1)"

		[ -z "$mtd_part_2" ] && reboot
		[ -z "$mtd_part_3" ] && reboot

		local bootconfig0rootfs=$(hexdump -v -e '1/1 "%01x|"' -n 1 -s 168 -C /dev/"$mtd_part_2" | cut -f 1 -d "|" | head -n1)
